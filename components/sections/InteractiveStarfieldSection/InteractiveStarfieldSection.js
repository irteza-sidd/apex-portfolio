...
export const InteractiveStarfieldSection = () => {
    return (<section className="relative w-full min-h-[1000px] bg-neutral-950 overflow-hidden">
      ...
      <img className="absolute top-px left-0 w-full h-full object-cover pointer-events-none" alt="Stars" src="/stars.svg"/>
      ...
      <div className="relative z-10 flex flex-col items-start gap-[100px] w-full max-w-[1620px] mx-auto px-8 py-[115px]">
        ...
        <div className="flex items-center justify-end gap-[70px] w-full">
          {stats.map((stat, index) => (<div key={stat.value} className="flex items-center gap-[70px]">
              <div className="flex flex-col w-[210px] items-center justify-end gap-2">
                ...
                <div className={`self-stretch font-paragraph-medium font-[number:var(--paragraph-medium-font-weight)] text-[#909090] text-[length:var(--paragraph-medium-font-size)] tracking-[var(--paragraph-medium-letter-spacing)] leading-[var(--paragraph-medium-line-height)] [font-style:var(--paragraph-medium-font-style)]${index === stats.length - 1 ? " whitespace-nowrap" : ""}`}>
                  {stat.label}
                </div>
              </div>
              {/* Vertical divider between stats, not after the last one */}
              {index < stats.length - 1 && (<img className="w-px h-[73px]" alt="Vector" src="/vector-29.svg"/>)}
            </div>))}
        </div>
      </div>
    </section>);
