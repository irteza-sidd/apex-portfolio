export const CompanyIntroSection = () => {
    ...
    return (<section className="flex flex-col w-full items-center justify-center gap-[60px] px-[150px] py-[60px] bg-neutral-950">
      ...
      <div className="flex items-center justify-center relative self-stretch w-full">
        <div className="flex flex-col items-center gap-[23px]">
          ...
          <div className="inline-flex items-start gap-12">
            {row1Logos.map((logo, index) => (<div key={`row1-${index}`} className={`relative ${logo.width} ${logo.height}`}>
                {/* Custom purple mark for the 5th logo in row 1 */}
                {logo.customMark === "purple" && (<div className="w-[26.51%] top-[4.17%] left-0 rounded-[15.43px] border-[2.1px] border-solid border-[#6826da] absolute h-[91.67%]">
                ...
                {logo.logomark && (<img className={logo.logomark.className} alt="Logomark" src={logo.logomark.src}/>)}
                {/* Logotype image */}
                {logo.logotype && (<img className={logo.logotype.className} alt="Logotype" src={logo.logotype.src}/>)}
              </div>))}
          </div>

          {/* Row 2 - 6 logos (wider row) */}
          <div className="flex w-[1417.9px] items-center justify-center gap-12">
            {row2Logos.map((logo, index) => (<div key={`row2-${index}`} className={`relative ${logo.width} ${logo.height}`}>
                {/* Custom blue mark for the 3rd logo in row 2 */}
                {logo.customMark === "blue" && (<div className="w-[22.47%] top-[4.17%] left-0 absolute h-[91.67%]">
          ...
          <div className="inline-flex items-center gap-12">
            {row3Logos.map((logo, index) => (<div key={`row3-${index}`} className={`relative ${logo.width} ${logo.height}`}>
                {/* Standard logomark image */}
                {logo.logomark && (<img className={logo.logomark.className} alt="Logomark" src={logo.logomark.src}/>)}
                {/* Logotype image */}
                {logo.logotype && (<img className={logo.logotype.className} alt="Logotype" src={logo.logotype.src}/>)}
