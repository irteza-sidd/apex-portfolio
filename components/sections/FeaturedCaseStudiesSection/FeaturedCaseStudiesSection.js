...
export const FeaturedCaseStudiesSection = () => {
    return (<section className="flex flex-col items-center relative self-stretch w-full flex-[0_0_auto]">
      {/* Header */}
      ...
      <div className="flex flex-col w-full max-w-[1620px] items-center gap-[30px] relative flex-[0_0_auto] mt-[-52px]">
        {caseStudies.map((study) => (<div key={study.id} className="flex flex-col items-start gap-2.5 pt-[260px] pb-0 px-0 relative self-stretch w-full flex-[0_0_auto]">
            <Card className={`flex h-[660px] items-center gap-12 p-8 relative self-stretch w-full ${study.bg} rounded-xl border border-solid ${study.borderColor} shadow-none`}>
              <CardContent className="flex items-center gap-12 p-0 w-full h-full">
                {/* Image on the left for study 2 */}
                {study.imagePosition === "left" && (<div className="relative w-[754px] h-[596px] rounded-2xl overflow-hidden bg-cover bg-[50%_50%] flex-shrink-0" style={{ backgroundImage: `url(${study.imageUrl})` }}/>)}

                {/* Text content */}
                <div className="flex flex-col items-end justify-between p-8 relative flex-1 self-stretch grow">
                  {/* Tags */}
                  <div className="inline-flex items-center gap-2.5 relative flex-[0_0_auto]">
                    {study.tags.map((tag, tagIndex) => (<div key={tagIndex} className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 relative flex-[0_0_auto] rounded-lg border border-solid ${tag.borderColor}`}>
                        <div className={`relative w-2 h-2 ${tag.dotColor} rounded`}/>
                        <span className={`relative w-fit mt-[-1.00px] [font-family:'Inter_Display-Regular',Helvetica] font-normal ${tag.textColor} text-[15px] tracking-[-0.15px] leading-[normal] whitespace-nowrap`}>
                          {tag.label}
                        </span>
                      </div>))}
                  </div>
                  ...
                  <div className="flex flex-col items-start gap-12 relative self-stretch w-full flex-[0_0_auto]">
                    <h3 className={`relative self-stretch mt-[-1.00px] ${study.titleColor} ${study.titleFont}`}>
                      {study.title}
                    </h3>

                    <img className="relative self-stretch w-full h-px" alt="Line" src="/line-1-1.svg"/>

                    {/* Stats row */}
                    <div className="flex items-center justify-between relative self-stretch w-full flex-[0_0_auto]">
                      {study.stats.map((stat, statIndex) => (<div key={statIndex} className="contents">
                          <div className="inline-flex flex-col items-start gap-1.5 relative flex-[0_0_auto]">
                            <span className={`self-stretch font-h2 ${stat.valueColor} text-[length:var(--h2-font-size)] tracking-[var(--h2-letter-spacing)] leading-[var(--h2-line-height)] relative mt-[-1.00px] font-[number:var(--h2-font-weight)] [font-style:var(--h2-font-style)]`}>
                              {stat.value}
                            </span>
                            <span className={`relative self-stretch font-paragraph-medium font-[number:var(--paragraph-medium-font-weight)] ${stat.labelColor} text-[length:var(--paragraph-medium-font-size)] tracking-[var(--paragraph-medium-letter-spacing)] leading-[var(--paragraph-medium-line-height)] [font-style:var(--paragraph-medium-font-style)]`}>
                              {stat.label}
                            </span>
                          </div>
                          {statIndex < study.stats.length - 1 && (<img className="relative w-px h-11" alt="Line" src="/line-2.svg"/>)}
                        </div>))}
                    </div>
                  </div>
                </div>
                ...
                {study.imagePosition === "right" && (<div className="relative w-[754px] h-[596px] rounded-2xl overflow-hidden bg-cover bg-[50%_50%] flex-shrink-0" style={{ backgroundImage: `url(${study.imageUrl})` }}/>)}
              </CardContent>
            </Card>
          </div>))}
        ...
        <div className="relative w-full bg-[linear-gradient(180deg,rgba(15,15,15,0)_0%,rgba(10,10,10,1)_100%)] flex items-end pb-0">
          <Card className="w-full gap-7 p-3.5 bg-[#ffffff] flex items-center rounded-xl shadow-none border-0">
            <CardContent className="flex items-center gap-7 p-0 w-full">
              {/* Thumbnail */}
              <div className="relative self-stretch w-[135px] rounded-lg bg-cover bg-[50%_50%] min-h-[80px]" style={{ backgroundImage: "url(/rectangle-2.png)" }}/>
              ...
              {/* Arrow icon */}
              <img className="relative w-[70px] h-[70px] flex-shrink-0" alt="Frame" src="/frame-1321325373-2.svg"/>
            </CardContent>
          </Card>
        </div>
      </div>
